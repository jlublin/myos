...
one page size is 12 bits and one uint32 size is 5 bits = 17 bits
...
uint32_t phys_mem_data[RAM_SIZE >> 17];
...
void init_phys_mm()
{
	main_memory.base_address = (phys_addr_t) RAM_BASE;
	main_memory.ram_size = RAM_SIZE; // 64 MB in bytes
...
/*
	...
	returns: nothing
*/
void dealloc_physical_page(phys_addr_t address)
{
